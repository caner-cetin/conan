...
class Classifier:
    ...
    def instrumental_or_vocal(self, audio: NDArray[np.float32]) -> list[float]:
        ...
        """
        effnet_output = self.effnet_model(audio)
        return np.mean(self.instrumental_or_vocal_model(effnet_output), axis=0).tolist()

    def vocal_gender(self, audio: NDArray[np.float32]) -> list[float]:
        ...
        """
        effnet_output = self.effnet_model(audio)
        return np.mean(self.vocal_gender_model(effnet_output), axis=0).tolist()

    def engaging(self, audio: NDArray[np.float32]) -> list[float]:
        ...
        """
        effnet_output = self.effnet_model(audio)
        return np.mean(self.engaging_model(effnet_output), axis=0).tolist()
