...
class TrackDisplayTemplate:
    ...
    def update_display(self, features: AudioFeatures, similar_tracks: list[tuple[str, float]], label: str) -> str:
        # Convert genre data to JSON strings for JavaScript
        genre_labels = json.dumps([label for label, _ in zip(features.genre_labels, features.genre_probabilities)])
        genre_probabilities = json.dumps([prob for _, prob in zip(features.genre_labels, features.genre_probabilities)])
